...
# One-shot pipeline for *multispectral* RTMSFDETR (RGB+MSI):
#   1) export dense fused features (per-image npz)
...
#   6) generate side-by-side panel (RGB+GT vs oil-heatmap overlay)
#   7) generate analysis charts/reports
#
# Usage:
#   bash tools/cluster_analysis/run_repr_oil_pipeline_rgb_msi.sh <train_run_dir> <dataset_root> [split] [device] [run_name]
#
# Example:
#   bash tools/cluster_analysis/run_repr_oil_pipeline_rgb_msi.sh \
...
#   STRICT_PAIRS=0                   (default: 0; 1 enables --strict-pairs in export)
...
#   PRED_AMP=0                       (default: 0; 1 enables --pred-amp; note: export AMP is separate)
...
if [[ -n "${RUN_NAME_ARG}" ]]; then
  ...
  TS="$(date +%Y%m%d-%H%M%S)"
  RUN_NAME="oil_${SPLIT}_${FEAT_SOURCE}${FEAT_LEVEL}_k${K1}x${K2}_rgb_msi_${TS}"
fi
...
echo "==== repr oil pipeline (RGB+MSI) ===="
...
echo "[1/7] export dense fused features -> ${RUN_DIR}/features"
EXPORT_CMD=(python tools/cluster_analysis/repr_export_rtmsfdetr_rgb_msi.py
