...
def _ensure_coco_dataset_keys_for_loadres(coco_gt: COCO) -> None:
    """
    pycocotools.COCO.loadRes 假设 GT COCO JSON 中存在 dataset['info']。
    部分自制/裁剪版 COCO 标注可能缺失该字段，导致 KeyError: 'info'。
    """
    ...
    if "info" not in dataset:
        dataset["info"] = {}
        logger.warning("COCO GT 标注缺失 'info' 字段，已注入空 dict 以兼容 pycocotools.loadRes()。")
...
class CocoEvaluator(object):
    def __init__(self, coco_gt, iou_types):
        ...
        _ensure_coco_dataset_keys_for_loadres(coco_gt)
