# 论文写法：Shallow Multispectral Feature Correction Module（SMFCM）

本节把项目中的
`origstem_residual_msbranch_shared_hgstem_inner_fixed_band_cmda_b3_stem_cf_interactive`
方法，对齐为论文写法，并将其论文名称统一表述为
`Shallow Multispectral Feature Correction Module (SMFCM)`。
本文给出其清晰的功能边界、张量符号、公式流程，以及它与
`MS-Stem`/`CRGGA`/直接改写 stem 的区别。

> 命名约定：本文中用
> - `SMFCM` 指 `Shallow Multispectral Feature Correction Module`；
...
- SMFCM 在 backbone 中的构造与注册：`engines/models/rtmsfdetr/rtdetrv4/engine/backbone/hgnetv2.py:582-753`
- SMFCM 在 backbone `forward` 中的执行路径：`engines/models/rtmsfdetr/rtdetrv4/engine/backbone/hgnetv2.py:1306-1345`
...
### 0.1 为什么命名为 SMFCM，而不是 Alignment/Interaction Module

论文名称统一采用：

\[
\text{Shallow Multispectral Feature Correction Module (SMFCM)}.
\]

这样命名的原因是：

1. **不是纯 Alignment Module。**  
   整个方法里，真正执行显式 band-wise 对齐的是残差 MS 分支内部的 `FixedBandCMDA`；而模块整体还包含后续的 `StemCF` 支路间交互与最终残差注入。因此，若把整个方法直接命名为 `Alignment Module`，会把审稿人的注意力过度集中到“是否做了严格几何配准”上，而弱化其“主路径浅层特征保守修正”的整体目标。

2. **不是纯 Interaction Module。**  
   `interaction` 只准确描述了 `StemCF` 这一个子步骤，即主 stem 特征作为 query、残差分支特征作为 memory 的 cross-attention 读取过程。它不能覆盖残差分支内部的显式 band-wise 对齐，也不能覆盖最终的小尺度 residual correction。

3. **`Feature` 比 `Spectral` 单独出现更准确。**  
   当前方法作用在 C2/stem 尺度的特征图上，而不是原始图像空间上的辐射校准或传统光谱响应校正。因此，论文名称中应明确是 `feature correction`，而不是容易被理解为传感器层/成像层“光谱校正”的 `spectral correction`。

4. **`Correction` 最能概括该方法的总体目标。**  
   该模块的本质不是重建一个新 stem，也不是强行做全局严格配准，而是通过：
   - 残差分支内部的波段错位修正；
   - 主路径与残差支路之间的保守交互；
   - 最终的小尺度残差注入  
   来对原始主路径的浅层多光谱表征进行整体修正。因此 `correction` 比 `alignment` 或 `interaction` 更能准确覆盖模块的最终功能边界。

简言之，`SMFCM` 的命名强调的是：

> 它是一个作用在浅层多光谱特征上的保守修正模块，其中“对齐”和“交互”都只是实现这一修正目标的子机制。
...
flowchart LR
    ...
    subgraph Correction["SMFCM Correction"]
...
代码定位：`StemBlock` 定义位于 `engines/models/rtmsfdetr/rtdetrv4/engine/backbone/hgnetv2.py:188-239`；在当前 SMFCM 配置下，主路径先执行原始 `self.stem(stem_input)`，再与残差支路汇合的逻辑位于 `engines/models/rtmsfdetr/rtdetrv4/engine/backbone/hgnetv2.py:1328-1345`。
...
因此，`FixedBandCMDA` 在整个 SMFCM 中承担的是：

1. **残差支路内部的显式波段错位修正器。**
2. **从显式 band 特征到“可被主路径利用的几何补充特征”的生成器。**
3. **整个方法中唯一明确对应“alignment”语义的子模块。**

换句话说，`FixedBandCMDA` 的作用不是定义整个方法，而是负责 SMFCM 里的“对齐子过程”。
...
这也是为什么论文名称最终采用 `Feature Correction`：  
整个模块的最终输出不是“一个对齐后的 band 集合”，也不是“一个单独的交互特征”，而是**被整体修正后的浅层多光谱主路径表示**。
...
> `SMFCM` 是一个面向浅层多光谱特征的保守校正模块，其中 `FixedBandCMDA` 负责残差支路内部的显式 band-wise 对齐与校正，`StemCF` 负责主路径与残差支路之间的交互式修正，而最终 residual injection 则完成对主路径表示的整体浅层多光谱特征校正。
