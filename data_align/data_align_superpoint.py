...
def process_sample(
    ...
    stage1_method: str,
    stage2_method: str,
    superpoint_weights: Optional[Path],
    superpoint_device: str,
    superpoint_nms_radius: int,
    superpoint_keypoint_threshold: float,
    superpoint_max_keypoints: int,
    superpoint_remove_border: int,
    superpoint_match_ratio: float,
    superpoint_cross_check: bool,
    superpoint_max_matches: int,
    superpoint_ransac_reproj_threshold: float,
    superpoint_min_inliers: int,
    superpoint_min_scale: float,
    superpoint_max_scale: float,
    ecc_motion: str,
    ecc_iters: int,
    ecc_eps: float,
    ...
    if enable_secondary_alignment and (stage1_method != "none" or stage2_method != "none"):
        ...
        for key, preserved_image, feature_gray, mask_preserved in feature_targets:
            ...
            elif stage1_method == "superpoint":
                if superpoint_weights is None:
                    raise ValueError(
                        "Missing --superpoint-weights for stage1_method=superpoint."
                    )
                m1 = estimate_superpoint_affine_transform(
                    ref_gray,
                    target_gray,
                    ref_mask=ref_mask_preserved,
                    tgt_mask=mask_preserved,
                    weights_path=superpoint_weights,
                    device=superpoint_device,
                    nms_radius=superpoint_nms_radius,
                    keypoint_threshold=superpoint_keypoint_threshold,
                    max_keypoints=superpoint_max_keypoints,
                    remove_border=superpoint_remove_border,
                    match_ratio=superpoint_match_ratio,
                    cross_check=superpoint_cross_check,
                    max_matches=superpoint_max_matches,
                    ransac_reproj_threshold=superpoint_ransac_reproj_threshold,
                    min_inliers=superpoint_min_inliers,
                    min_scale=superpoint_min_scale,
                    max_scale=superpoint_max_scale,
                )
                if m1 is not None:
                    apply_refine_current(m1)
                else:
                    LOGGER.debug("Stage1 SuperPoint failed for %s", key)
    ...
    for image_path in image_files:
        ...
        try:
            full_stack, nir_stack, channel_labels = process_sample(
                ...
                superpoint_weights,
                superpoint_device,
                superpoint_nms_radius,
                superpoint_keypoint_threshold,
                superpoint_max_keypoints,
                superpoint_remove_border,
                superpoint_match_ratio,
                superpoint_cross_check,
                superpoint_max_matches,
                superpoint_ransac_reproj_threshold,
                superpoint_min_inliers,
                superpoint_min_scale,
                superpoint_max_scale,
                ecc_motion,
                ecc_iters,
                ecc_eps,
                crop_black_edges,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--stage1-method",
        choices=["none", "orb", "superpoint"],
        default="none",
        help="Secondary stage1 method (coarse): none/orb/superpoint.",
    )
    parser.add_argument(
        "--stage2-method",
        choices=["none", "ecc"],
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Path to SuperPoint .pth weights (required when --stage1-method superpoint).",
    )
    parser.add_argument(
        "--superpoint-device",
        type=str,
        default="cuda",
        help="Torch device for SuperPoint inference (e.g. cuda/cpu).",
    )
    parser.add_argument(
        "--superpoint-nms-radius",
        type=int,
        default=4,
        help="SuperPoint NMS radius (pixels).",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=2048,
        help="Max SuperPoint keypoints per image (<=0 means keep all).",
    )
    parser.add_argument(
        "--superpoint-remove-border",
        type=int,
        default=4,
        help="Remove SuperPoint keypoints within this border (pixels).",
    )
    parser.add_argument(
        "--superpoint-match-ratio",
        type=float,
        default=0.9,
        help="Descriptor matching ratio test threshold.",
    )
    parser.add_argument(
        "--superpoint-cross-check",
        action="store_true",
        help="Enable mutual nearest-neighbor check for matches.",
    )
    parser.add_argument(
        "--superpoint-max-matches",
        type=int,
        default=500,
        help="Keep at most this many matches (<=0 means keep all).",
    )
    parser.add_argument(
        "--superpoint-ransac-reproj-threshold",
        type=float,
        default=3.0,
        help="RANSAC reprojection threshold (pixels) for SuperPoint affine estimation.",
    )
    parser.add_argument(
        "--superpoint-min-inliers",
        type=int,
        default=8,
        help="Minimum inlier matches for accepting SuperPoint affine.",
    )
    parser.add_argument(
        "--superpoint-min-scale",
        type=float,
        default=0.7,
        help="Reject SuperPoint affine if estimated scale is below this bound.",
    )
    parser.add_argument(
        "--superpoint-max-scale",
        type=float,
        default=1.3,
        help="Reject SuperPoint affine if estimated scale is above this bound.",
    )
    parser.add_argument(
        "--ecc-motion",
...
def main() -> None:
    ...
    # - If user passes --enable-secondary-align without specifying any stage method,
    #   default to stage1=orb, stage2=none (original ORB-only secondary alignment).
    stage1_method = args.stage1_method
    stage2_method = args.stage2_method
    if args.enable_secondary_align and stage1_method == "none" and stage2_method == "none":
        stage1_method = "superpoint" if args.superpoint_weights is not None else "orb"

    enable_secondary = args.enable_secondary_align or stage1_method != "none" or stage2_method != "none"

    if stage1_method == "superpoint" and args.superpoint_weights is None:
        raise SystemExit("--superpoint-weights is required when --stage1-method superpoint.")
    ...
    align_dataset(
        ...
        superpoint_device=args.superpoint_device,
        superpoint_nms_radius=args.superpoint_nms_radius,
        superpoint_keypoint_threshold=args.superpoint_keypoint_threshold,
        superpoint_max_keypoints=args.superpoint_max_keypoints,
        superpoint_remove_border=args.superpoint_remove_border,
        superpoint_match_ratio=args.superpoint_match_ratio,
        superpoint_cross_check=args.superpoint_cross_check,
        superpoint_max_matches=args.superpoint_max_matches,
        superpoint_ransac_reproj_threshold=args.superpoint_ransac_reproj_threshold,
        superpoint_min_inliers=args.superpoint_min_inliers,
        superpoint_min_scale=args.superpoint_min_scale,
        superpoint_max_scale=args.superpoint_max_scale,
