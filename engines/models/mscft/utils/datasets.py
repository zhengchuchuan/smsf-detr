...
from engines.models.mscft.utils.general import (
    check_requirements,
    xyxy2xywh,
    xywh2xyxy,
    xywhn2xyxy,
...
from engines.models.mscft.utils.torch_utils import torch_distributed_zero_first


class _LocalSamplerState:
    """轻量级全局存储，用于替代已移除的 third_party.global_var 依赖。"""
...
try:
    import global_var as _external_global_var  # type: ignore
except Exception:
    _external_global_var = None

_SAMPLER_STATE = _LocalSamplerState()
...
class RandomSampler(torch.utils.data.sampler.RandomSampler):
    ...
    def __iter__(self):
        ...
        indices = None
        if _external_global_var is not None and hasattr(_external_global_var, "get_value"):
            try:
                indices = _external_global_var.get_value("s")
            except Exception:
                indices = None
        if indices is None:
            indices = _SAMPLER_STATE.get("s")
        if indices is None:
            indices = torch.randperm(n).tolist()
            _SAMPLER_STATE.set("s", indices)
        return iter(indices)
