...
class PositionEmbeddingSine(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor, align_dim_orders = True):
        # 对 mask 取反后沿 y/x 方向累加即可得到“像素所在有效区域中的相对坐标”，这是最核心的步骤
        ...
        # 采用固定频率的正弦/余弦编码，偶数通道为 sin，奇数通道为 cos
    ...
    def forward_export(self, mask:torch.Tensor, align_dim_orders = True):
        # 导出模式与 forward 完全一致，只是输入改为裸 mask，避免对 NestedTensor 的依赖
        assert mask is not None
        not_mask = ~mask
        y_embed = not_mask.cumsum(1, dtype=torch.float32)
        x_embed = not_mask.cumsum(2, dtype=torch.float32)
...
class PositionEmbeddingLearned(nn.Module):
    ...
    def forward(self, tensor_list: NestedTensor):
        # 直接通过行/列 embedding 网格化生成绝对位置特征，适用于输入尺寸固定的场景
        x = tensor_list.tensors
        h, w = x.shape[:2]
...
def build_position_encoding(hidden_dim, position_embedding):
    # 约定总通道数的一半用于正弦/学习式坐标，每个坐标维度会再拆成 sin/cos 两个通道
