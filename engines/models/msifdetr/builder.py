...
from engines.models.msifdetr.common.backbone import build_backbone
from engines.models.msifdetr.common.matcher import build_matcher
from engines.models.msifdetr.common.transformer import build_transformer
from engines.models.msifdetr.common.segmentation_head import (
    SegmentationHead,
    get_uncertain_point_coords_with_randomness,
    point_sample,
)

class LWDETR(BaseDetector):
    """ This is the Group DETR v3 module that performs object detection """

    supported_modalities = ("rgb", "ms")
    ...
    def __init__(self,
                 ...
                 bbox_reparam=False):
        """初始化 LW-DETR 检测头：backbone -> transformer -> 分类/回归/分割。"""
        super().__init__()
        ...
        # 可选的解码器参考点自回归；lite_refpoint_refine=True 时解码器不再迭代更新 bbox_embed。
        ...
        # focal loss 的先验偏置，降低早期正样本梯度。
        prior_prob = 0.01
        bias_value = -math.log((1 - prior_prob) / prior_prob)
        self.class_embed.bias.data = torch.ones(num_classes) * bias_value

        # 将 bbox 回归最后一层初始化为零，保证初始输出接近 refpoint。
    ...
    def reinitialize_detection_head(self, num_classes):
        # 重置分类头以适配新类别数，同时同步 two_stage 的编码器头。
    ...
    def forward(self, samples: NestedTensor, targets=None):
        ...
        if isinstance(samples, dict):
            ...
            for key, value in samples.items():
                if isinstance(value, NestedTensor):
                    ...
                    normalized[key] = nested_tensor_from_tensor_list(value)
            samples = normalized
        elif isinstance(samples, (list, torch.Tensor)):
            samples = nested_tensor_from_tensor_list(samples)
        # backbone 输出特征列表与位置编码
        backbone_out = self.backbone(samples)
        ms_features = None
        ms_poss = None
        if isinstance(backbone_out, (list, tuple)) and len(backbone_out) == 4:
            features, poss, ms_features, ms_poss = backbone_out
        else:
            features, poss = backbone_out
        ...
        if self.training:
            ...
            # 推理仅使用单组 query，避免多组重复
            refpoint_embed_weight = self.refpoint_embed.weight[:self.num_queries]
            query_feat_weight = self.query_feat.weight[:self.num_queries]

        # Transformer 返回解码器输出 hs、解码器参考点、编码器输出 hs_enc 与编码器参考点
        if ms_features is not None and ms_poss is not None:
            ms_srcs = []
            ms_masks = []
            ...
            hs, ref_unsigmoid, hs_enc, ref_enc = self.transformer(
                ...
                ms_srcs=ms_srcs, ms_masks=ms_masks, ms_pos_embeds=ms_poss,
            )
        else:
            hs, ref_unsigmoid, hs_enc, ref_enc = self.transformer(
                srcs, masks, poss, refpoint_embed_weight, query_feat_weight)
        ...
        if hs is not None:
            ...
            if self.training:
                # 训练阶段不应静默吞掉 NaN/Inf，否则会污染权重且指标可能长期为 0。
                if (not torch.isfinite(outputs_coord).all()) or (not torch.isfinite(outputs_class).all()):
                    raise FloatingPointError("MSIF-DETR forward 产生 NaN/Inf，请检查 AMP/学习率/融合配置。")
            else:
                # 推理阶段容错：避免导出/部署时因极端输入产生 NaN。
                outputs_coord = torch.nan_to_num(outputs_coord)
                outputs_class = torch.nan_to_num(outputs_class)

            if self.segmentation_head is not None:
                outputs_masks = self.segmentation_head(features[0].tensors, hs, samples.tensors.shape[-2:])

            # 取最后一层解码器输出作为最终结果，可选返回多层 aux loss
        ...
        if self.two_stage:
            if self.training and (ref_enc is not None) and (not torch.isfinite(ref_enc).all()):
                raise FloatingPointError("MSIF-DETR two_stage ref_enc 产生 NaN/Inf。")
            ref_enc = torch.nan_to_num(ref_enc) if (ref_enc is not None and not self.training) else ref_enc
            # 编码器输出第一阶段提案（按 group 拆分再合并），可用于辅助损失或无解码器场景。
            group_detr = self.group_detr if self.training else 1
            hs_enc_list = hs_enc.chunk(group_detr, dim=1)
            cls_enc = []
            for g_idx in range(group_detr):
                cls_enc_gidx = self.transformer.enc_out_class_embed[g_idx](hs_enc_list[g_idx])
                if self.training and (not torch.isfinite(cls_enc_gidx).all()):
                    raise FloatingPointError("MSIF-DETR two_stage cls_enc 产生 NaN/Inf。")
                if not self.training:
                    cls_enc_gidx = torch.nan_to_num(cls_enc_gidx)
    ...
    def forward_export(self, rgb: torch.Tensor, ms: torch.Tensor | None = None):
        # 导出模式下：
        # - RGB-only：rgb 为 (B,3,H,W)
        # - RGB+MSI：rgb 为 (B,3,H,W)，ms 为 (B,ms_ch,H,W)
        if ms is None:
            backbone_out = self.backbone(rgb)
        else:
            backbone_out = self.backbone(rgb, ms)

        if isinstance(backbone_out, (list, tuple)) and len(backbone_out) == 6:
            srcs, _, poss, ms_srcs, ms_masks, ms_poss = backbone_out
        else:
            srcs, _, poss = backbone_out
            ms_srcs = ms_masks = ms_poss = None

        # only use one group in inference
        refpoint_embed_weight = self.refpoint_embed.weight[:self.num_queries]
        query_feat_weight = self.query_feat.weight[:self.num_queries]

        if ms_srcs is not None and ms_poss is not None:
            hs, ref_unsigmoid, hs_enc, ref_enc = self.transformer(
                srcs,
                None,
                poss,
        ...
        if hs is not None:
            if self.bbox_reparam:
                ...
                outputs_coord = (self.bbox_embed(hs) + ref_unsigmoid).sigmoid()
            outputs_coord = torch.nan_to_num(outputs_coord)
            outputs_class = torch.nan_to_num(self.class_embed(hs))
            if self.segmentation_head is not None:
                outputs_masks = self.segmentation_head(srcs[0], [hs,], rgb.shape[-2:])[0]
        else:
            assert self.two_stage, "if not using decoder, two_stage must be True"
            outputs_class = torch.nan_to_num(self.transformer.enc_out_class_embed[0](hs_enc))
            outputs_coord = torch.nan_to_num(ref_enc)
            if self.segmentation_head is not None:
                outputs_masks = self.segmentation_head(srcs[0], [hs_enc,], rgb.shape[-2:], skip_blocks=True)[0]
...
def build_model(args):
    ...
    num_classes = args.num_classes + 1  # 多一个_background 类别
    device = torch.device(args.device)


    # 当启用 MSI 分支（P4 融合 / Decoder 侧融合）时，RGB backbone 始终应使用 RGB 通道数构建，
    # 即使数据侧选择“通道堆叠单输入”（dual_stream_output=false）也不例外；
    # 这种情况下 Joiner 会在前向中将 (rgb+ms) 拆分为两路，再分别送入 RGB/MSI 分支。
    ms_enabled = bool(getattr(args, "msi_fusion", False)) or bool(getattr(args, "decoder_ms_fusion", False))
    ms_channels = int(getattr(args, "ms_input_channels", 0))
    if ms_enabled and ms_channels > 0:
        backbone_input_channels = int(getattr(args, "rgb_input_channels", 3))
    else:
        backbone_input_channels = int(getattr(args, "input_channels", 3))
    ...
    backbone = build_backbone(
        ...
        target_shape=args.shape if hasattr(args, 'shape') else (args.img_size, args.img_size) if hasattr(args, 'img_size') else (640, 640),
        rms_norm=args.rms_norm,
        backbone_lora=args.backbone_lora,
        force_no_pretrain=args.force_no_pretrain,
        gradient_checkpointing=args.gradient_checkpointing,
        # 允许在加载 RF-DETR checkpoint 的同时继续加载 DINOv2 backbone 权重（用于结构不兼容时兜底）。
        ...
        # MSI 融合（P4 单点融合 + 门控）
        msi_fusion=bool(getattr(args, "msi_fusion", False)),
        decoder_ms_fusion=bool(getattr(args, "decoder_ms_fusion", False)),
        msi_backbone=str(getattr(args, "msi_backbone", "specat_lite")),
        ms_input_channels=int(getattr(args, "ms_input_channels", 7)),
        msi_pre_downsample_to_p4=bool(getattr(args, "msi_pre_downsample_to_p4", False)),
        specat_base_dim=int(getattr(args, "specat_base_dim", 64)),
        specat_stage=int(getattr(args, "specat_stage", 3)),
        specat_num_blocks=getattr(args, "specat_num_blocks", None),
        specat_dim_head=getattr(args, "specat_dim_head", None),
        specat_attention_type=str(getattr(args, "specat_attention_type", "base")),
        film_reduction=int(getattr(args, "film_reduction", 4)),
        film_spatial=bool(getattr(args, "film_spatial", False)),
        film_kernel_size=int(getattr(args, "film_kernel_size", 3)),
        fusion_alpha_init=float(getattr(args, "fusion_alpha_init", 0.0)),
        fusion_alpha_scale=float(getattr(args, "fusion_alpha_scale", 1.0)),
        freeze_fusion_alpha=bool(getattr(args, "freeze_fusion_alpha", False)),
        freeze_msi_backbone=bool(getattr(args, "freeze_msi_backbone", False)),
        use_ms_mask=bool(getattr(args, "use_ms_mask", False)),
    )
