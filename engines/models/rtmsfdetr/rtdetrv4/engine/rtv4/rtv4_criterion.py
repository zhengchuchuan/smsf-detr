...
class RTv4Criterion(nn.Module):
    ...
    def __init__(self, \
        ...
        # Cache the last Hungarian match indices for optional extra losses (e.g. instance segmentation).
        # This avoids recomputing matching in wrapper criteria.
    ...
    def loss_distillation(self, outputs, targets, indices, num_boxes, **kwargs):
        ...
        if student_feature_map is None or teacher_feature_map is None:
            # 蒸馏未启用/teacher 未提供时，返回 0 loss。
            # device 必须与当前输出一致，避免在 CPU 推理时硬编码到 cuda。
    ...
    def forward(self, outputs, targets, **kwargs):
        ...
        # Extra auxiliary losses produced by the model (e.g. deformable alignment in backbone).
        # Keep it weight_dict-driven when available; otherwise default to weight=1.0 for backward compatibility.
        for extra_key in (
            "loss_deform_align",
            "loss_deform_offset",
            "loss_deform_attn",
            ...
            if extra_key not in outputs:
                continue
            value = outputs.get(extra_key)
            if not torch.is_tensor(value):
                continue
            weight = float(self.weight_dict.get(extra_key, 1.0))
            losses[extra_key] = value * weight
