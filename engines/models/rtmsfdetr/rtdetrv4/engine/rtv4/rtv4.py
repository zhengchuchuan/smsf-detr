...
class RTv4(nn.Module):
    ...
    def forward(self, x, targets=None, teacher_encoder_output=None):
        ...
        aux_losses = None
        if isinstance(x_backbone, tuple) and len(x_backbone) == 2:
            x_backbone, aux_losses = x_backbone
        ...
        # Extra auxiliary losses computed in backbone (e.g. deformable alignment).
        if aux_losses is not None and isinstance(x_decoder_out, dict) and isinstance(aux_losses, dict):
