...
class TransformerDecoder(nn.Module):
    ...
    def forward(self,
        ...
        dec_out_refs = []
        dec_out_hs = [] if return_hs else None
        ...
        out_pred_corners = torch.stack(dec_out_pred_corners)
        out_refs = torch.stack(dec_out_refs)
        if return_hs:
            hs = torch.stack(dec_out_hs) if dec_out_hs is not None else out_bboxes.new_zeros((0,))
            return out_bboxes, out_logits, out_pred_corners, out_refs, pre_bboxes, pre_scores, hs
        return out_bboxes, out_logits, out_pred_corners, out_refs, pre_bboxes, pre_scores
...
class DFINETransformer(nn.Module):
    ...
    def forward(self, feats, targets=None):
        ...
        # decoder
        seg_head = getattr(self, "segmentation_head", None)
        return_hs = seg_head is not None
        if return_hs:
        ...
        if self.training and dn_meta is not None:
            ...
            if return_hs:
                _, hs = torch.split(hs, dn_meta['dn_num_split'], dim=2)
        ...
        if return_hs:
            # Predict masks from the selected FPN level and per-layer decoder hidden states.
            level = int(getattr(self, "mask_feature_level", 0) or 0)
            level = max(0, min(level, len(feats) - 1))
            spatial_features = self.input_proj[level](feats[level]) if hasattr(self, "input_proj") else feats[level]

            image_size = None
            if isinstance(targets, (list, tuple)) and len(targets) > 0 and isinstance(targets[0], dict):
                size = targets[0].get("size")
                if torch.is_tensor(size) and size.numel() >= 2:
                    image_size = (int(size[0].item()), int(size[1].item()))
            if image_size is None:
                stride = int(self.feat_strides[level]) if hasattr(self, "feat_strides") else 1
                image_size = (int(spatial_features.shape[-2] * stride), int(spatial_features.shape[-1] * stride))

            mask_logits = seg_head(spatial_features, hs, image_size)
            out["pred_masks"] = mask_logits[-1]

            if self.training and self.aux_loss and bool(getattr(self, "mask_aux_loss", False)):
                aux_list = out.get("aux_outputs") or []
                # aux_outputs corresponds to out_logits[:-1], i.e. all but last layer.
                for i, aux in enumerate(aux_list):
                    if i < len(mask_logits) - 1 and isinstance(aux, dict):
                        aux["pred_masks"] = mask_logits[i]
