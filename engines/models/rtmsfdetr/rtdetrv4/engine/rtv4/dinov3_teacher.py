...
class DINOv3TeacherModel(nn.Module):
    ...
    def forward(self, images: torch.Tensor):
        ...
        with torch.no_grad():
            ...
            # 避免每 step 打印导致日志爆炸：仅首次 forward 打印一次。
            if not getattr(self, "_logged_spatial_size", False):
                _logger.info("[Teacher Model] Spatial size: %s", tuple(teacher_feature_map.shape[2:]))
                self._logged_spatial_size = True
