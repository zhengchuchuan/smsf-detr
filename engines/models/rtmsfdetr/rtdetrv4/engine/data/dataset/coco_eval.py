...
try:
    ...
    import faster_coco_eval.core.mask as mask_util  # type: ignore
    _EVAL_IMGS_ATTR = "_evalImgs_cpp"
except ModuleNotFoundError:
    # `faster_coco_eval` is an optional acceleration dependency.
    # Fall back to the standard pycocotools evaluator.
    from pycocotools.coco import COCO  # type: ignore
    from pycocotools.cocoeval import COCOeval as _COCOeval  # type: ignore
    import pycocotools.mask as mask_util  # type: ignore

    _EVAL_IMGS_ATTR = "evalImgs"

    class COCOeval_faster(_COCOeval):  # type: ignore
        def __init__(self, cocoGt=None, cocoDt=None, iouType="segm", **_kwargs):
...
class CocoEvaluator(object):
    ...
    def update(self, predictions):
        ...
        for iou_type in self.iou_types:
            ...
            eval_imgs = getattr(coco_eval, _EVAL_IMGS_ATTR)
            self.eval_imgs[iou_type].append(
                np.array(eval_imgs).reshape(
    ...
    def synchronize_between_processes(self):
        for iou_type in self.iou_types:
            ...
            setattr(coco_eval, _EVAL_IMGS_ATTR, eval_imgs)
