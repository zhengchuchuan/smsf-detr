...
class DinoV2(nn.Module):
    def __init__(
        ...
        resolved_channels = num_input_channels if num_input_channels is not None else 3
        
        # Create the encoder
        
        if not use_windowed_attn:
            if num_input_channels not in (None, 3):
                raise ValueError(
                    "Custom input_channels is only supported for the windowed DINOv2 backbone."
                )
            ...
            base_channels = dino_config.get("num_channels", 3)
            desired_channels = num_input_channels if num_input_channels is not None else base_channels
            if desired_channels != base_channels:
                dino_config["num_channels"] = desired_channels
                if load_dinov2_weights:
                    logger.warning(
                        "Disabling DINOv2 pretrained weights because num_channels=%s (pretrained expects %s).",
                        desired_channels,
                        base_channels,
                    )
        ...
        self.input_channels = resolved_channels
        self._export = False
    ...
    def export(self):
        ...
        def make_new_interpolated_pos_encoding(
            ...
            patch_pos_embed = F.interpolate(
                ...
                mode="bicubic",
                align_corners=False,
                # ONNX 导出不支持 aten::_upsample_bicubic2d_aa（antialias=True）。
                # 该插值仅用于位置编码尺寸对齐，关闭 antialias 对推理影响可忽略。
                antialias=False,
            )
