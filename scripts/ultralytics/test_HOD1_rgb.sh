...
# If MODEL not set, pick the latest HOD1 run under outputs/ultralytics.
if [[ -z "${MODEL:-}" ]]; then
  MODEL="$(ls -t "$ROOT"/outputs/ultralytics/HOD1_rgb_*/weights/best.pt 2>/dev/null | head -n 1 || true)"
fi
if [[ -z "$MODEL" ]]; then
