#!/usr/bin/env bash
  ...
  # bash scripts/mrt-detr/train_hod1_rgb_msi.sh
  ...
  REPO_ROOT="$(pwd)"
  DATA_CFG="${REPO_ROOT}/configs/data/HOD1_rgb_msi.yaml"
  # 训练前生成 annotations_8cls/{train,val,test}.json（确保只包含 data.class_names）
  DST_ANN_DIR="${REPO_ROOT}/data/HOD-1/annotations_8cls"

  OUTDIR="${REPO_ROOT}/third_party/MRT-DETR/output/msifp_hod1_rgb_msi3"
  MRT_DIR="${REPO_ROOT}/third_party/MRT-DETR"
  MRT_CFG_REL="rtdetr_pytorch/configs/rtdetr/rtdetr_dual_msifp_hod1_rgb_msi3.yml"
  ...
  echo "[1/4] Build filtered annotations (annotations_8cls)..."
  ...
  LOG_FILE="${MRT_DIR}/output/train_msifp_hod1_rgb_msi3_$(date +%Y%m%d_%H%M%S).log"
  ...
  python third_party/MRT-DETR/rtdetr_pytorch/tools/plot_metrics.py \
    --output-dir third_party/MRT-DETR/output/msifp_hod1_rgb_msi3
